...
```{r}
look = decluster(wind_speed$surface_wind, threshold1, r=7)
look
...
look1 = decluster(wind_speed$surface_wind, threshold)
...
ci(fitBayes1, type="parameter")

# Posterior modes of the parameters
postmode(fitBayes1)
mu0_mode = postmode(fitBayes1)["mu0"]
mu1_mode = postmode(fitBayes1)["mu1"]
phi0_mode = postmode(fitBayes1)["phi0"]
phi1_mode = postmode(fitBayes1)["phi1"]
xi0_mode = postmode(fitBayes1)["xi0"]
xi1_mode = postmode(fitBayes1)["xi1"]

# Get return period using posterior mode
return_period = 500
postModeReturnLevel_summer = (mu0_mode + mu1_mode) - (exp(phi0_mode + phi1_mode) / (xi0_mode + xi1_mode)) * ((1-(-log(1 - 1/return_period))^(-(xi0_mode + xi1_mode))))
postModeReturnLevel_nonsummer = mu0_mode - (exp(phi0_mode) / xi0_mode) * ((1-(-log(1 - 1/return_period))^(-xi0_mode)))
...
cdf_gev = function(x) {
  ...
  xi_0 = x["xi0"]
  xi_1 = x["xi1"]
  return(pevd(12.41, loc = mu_0 + mu_1, scale = exp(phi_0 + phi_1), shape = xi_0 + xi_1, type = "PP"))
}
