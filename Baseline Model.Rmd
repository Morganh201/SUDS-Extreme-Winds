...
```{r}
wind_speed = read.csv("pearson_wind_speed.csv")
...
yearly_mean_climatology = wind_speed %>% group_by(day_of_year) %>% summarise(mean_wind_speed = mean(wind_speed))
...
```{r}
ggplot(wind_speed, aes(x = day_of_year, y = wind_speed)) + 
...
```{r}
yearly_max_climatology = wind_speed %>% group_by(day_of_year) %>% summarise(max_wind_speed = max(wind_speed))
...
```{r}
hist(wind_speed$wind_speed, freq=FALSE, breaks = 30)
lines(density(wind_speed$wind_speed), col='red', lwd = 2)
```
...
```{r, warning=FALSE}
weibull_fit = fitdistr(wind_speed$wind_speed, "weibull")
weibull_fit
```

```{r}
# Sort the data
data_sorted <- sort(wind_speed$wind_speed)
...
# Plot boxplots
ggplot(extreme_winds, aes(x = Period, y = wind_speed, fill = Period)) + geom_boxplot() +
...
ggplot(extreme_winds, aes(x = Year, y = wind_speed)) + geom_point() +
...
ggplot(extreme_winds, aes(x = wind_speed, fill = Period)) +
...
wilcox.test(previous$wind_speed, current$wind_speed, 
...
```{r}
threshold = quantile(wind_speed$wind_speed, probs = 0.98)
fit = fevd(wind_speed, wind_speed, threshold = threshold, type = "GP")
plot(fit)
plot(fit, "trace")
...
```{r, warning=FALSE}
threshrange.plot(wind_speed$wind_speed, r = c(3, 12), nint = 25)
mrlplot(wind_speed$wind_speed, xlim=c(3, 12))
```
...
```{r}
fit1 = fevd(wind_speed, wind_speed, threshold = 8.5, type = "GP")
plot(fit1)
plot(fit1, "trace")
...
```{r}
emp_cdf = ecdf(wind_speed$wind_speed)
cdf_value = emp_cdf(8.5)
atdf(wind_speed$wind_speed, cdf_value)
```
...
```{r}
threshold1 = 8.5
extremalindex(wind_speed$wind_speed, threshold1)
```
...
```{r}
look = decluster(wind_speed$wind_speed, threshold1, r=7)
look
plot(wind_speed$wind_speed[2000:2200], type='o')
...
```{r}
extreme_winds = wind_speed %>% filter(wind_speed > 8.5)

# Plot boxplots
ggplot(extreme_winds, aes(x = factor(Month), y = wind_speed, fill = factor(Month))) + geom_boxplot() +
  labs(title = "Wind Speed Boxplots",
       x = "Time Period",
       y = "Wind Speed (m/s)") +
  theme_minimal()
...
# Plot violin plots
ggplot(extreme_winds, aes(x = factor(Month), y = wind_speed, fill = factor(Month))) + geom_violin() +
...
wind_speed = wind_speed %>% mutate(summer_index = case_when(Month %in% c(6, 7, 8) ~ 1,
                                                      TRUE ~ 0))
declustered_wind = data.frame(wind_speed = c(look), valid_time = wind_speed$valid_time, 
                              summer_index = wind_speed$summer_index)
fit3 = fevd(wind_speed, declustered_wind, scale.fun = ~ summer_index, threshold = 8.5, 
            use.phi = TRUE, type="GP")
fit4 = fevd(wind_speed, declustered_wind, scale.fun = ~ summer_index, shape.fun = ~summer_index,
...
threshrange.plot(non_summer_wind$wind_speed, r = c(3, 12), nint = 35)
mrlplot(non_summer_wind$wind_speed, xlim=c(3, 12))
threshrange.plot(summer_wind$wind_speed, r = c(3, 9), nint = 35)
mrlplot(summer_wind$wind_speed, xlim=c(3, 9))
```
...
look1 = decluster(wind_speed$wind_speed, threshold)
season_declustered_wind = data.frame(wind_speed = c(look1), valid_time = wind_speed$valid_time, 
                              summer_index = wind_speed$summer_index)
```

```{r}
fit5 = fevd(wind_speed, season_declustered_wind, threshold = c(9.6, -1.9), threshold.fun = ~ summer_index,
            type="PP")
fit6 = fevd(wind_speed, season_declustered_wind, threshold = c(9.6, -1.9), threshold.fun = ~ summer_index,
            location.fun = ~ summer_index, type="PP")
fit7 = fevd(wind_speed, season_declustered_wind, threshold = c(9.6, -1.9), threshold.fun = ~ summer_index,
            location.fun = ~ summer_index, scale.fun = ~ summer_index, use.phi = TRUE, type="PP")
fit8 = fevd(wind_speed, season_declustered_wind, threshold = c(9.6, -1.9), threshold.fun = ~ summer_index,
...
plot(fitBayes, rperiods = c(100), type = 'rl')
plot(fitBayes, "trace")
...
bayesian_return_level_ci <- function(fit, summer_index, return.period = 100) {
  ...
  ci = tibble("95% lower CI" = lower_ci, Estimate = mean(return_levels), "95% upper CI" = upper_ci)
  return(ci)
}
```

```{r, warning = FALSE}
fitBayes1 = fevd(wind_speed, season_declustered_wind, threshold = c(9.6, -1.9), 
                threshold.fun = ~ summer_index, location.fun = ~ summer_index, scale.fun = ~ summer_index,
                shape.fun = ~ summer_index, type="PP", use.phi = TRUE, method="Bayesian", 
                iter = 10000, verbose=TRUE)
plot(fitBayes1, rperiods = c(100), type = 'rl')
plot(fitBayes1, "trace")
bayesian_return_level_ci(fitBayes1, summer_index = 1, return.period = 500)
```

From our MCMC iterations, we can see that our confidence intervals of return levels are not symmetric anymore due to the non-symmetry of a couple of posterior distributions of parameters, especially the shape parameter. However, the CIs using the Bayesian technique seem to be shifted to the right compared to the effective return levels of the frequentist method. We compare the parameter values and the posterior mean/mode of both methods:
```{r}
ci(fit7, type = "parameter")
ci(fit8, type="parameter")

# Posterior modes of the parameters
postmode(fit8)
mu0_mode = postmode(fit8)["mu0"]
mu1_mode = postmode(fit8)["mu1"]
phi0_mode = postmode(fit8)["phi0"]
phi1_mode = postmode(fit8)["phi1"]
shape_mode = postmode(fit8)["shape"]

# Get return period using posterior mode
return_period = 500
postModeReturnLevel_summer = (mu0_mode + mu1_mode) - (exp(phi0_mode + phi1_mode) / shape_mode) * ((1-(-log(1 - 1/return_period))^(-shape_mode)))
postModeReturnLevel_nonsummer = mu0_mode - (exp(phi0_mode) / shape_mode) * ((1-(-log(1 - 1/return_period))^(-shape_mode)))

paste("CI estimate for posterior mode nonsummer:", postModeReturnLevel_nonsummer)
paste("CI estimate for posterior mode summer:", postModeReturnLevel_summer)
```

We see that using the posterior modes for our return level calculation makes their estimates closer to the non-Bayesian estimates. A better, more natural way of calculating the confidence intervals of the return levels however is to calculate the predictive density. Since return levels concern itself with predictions of the future, this method is a more natural way of estimation. Moreover, predictive densities are able to capture the uncertainty of a future prediction as well as the uncertainty of the model estimates. Notice that using only the posterior estimates will only consider the uncertainty of the parameter estimates. Calculating the predictive density is not possible using functions in R packages, so we will only provide an approximation using a brute force approach(Possible source in calculating the distribution: https://stats.stackexchange.com/questions/558672/approximate-posterior-predictive-quantiles-with-numerical-methods, https://cran.r-project.org/web/packages/mvQuad/vignettes/mvQuad_intro.html)
```{r}
...
cdf_gev = function(x) {
  ...
  phi_1 = x["phi1"]
  shape = x["shape"]
  return(pevd(12.41, loc = mu_0, scale = exp(phi_0), shape = shape, type = "PP"))
}

1/2
mean(apply(fit8$results, MARGIN = 1, FUN = cdf_gev)[500:10000])
```

With a significantly higher effective return level in non-summer months of 18.3656 and 12.55695 in summer months for a return period of 500 years after calculating the approximate posterior predictive distribution, this could be due to an inaccurate calculation of the distribution, or it could reflect the amount of uncertainty that has been considered in the calculation, especially since we have less than 50 years of data available. Notice that the difference is much smaller when calculating the return level of a return period of 2 years, with less than 0.01 difference from the posterior mean estimate. Thus, it is likely that the large discrepancy is due to the large uncertainties inherited by a lack of long-term data. 
