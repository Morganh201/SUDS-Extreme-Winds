...
ci(fit8, type="return.level", return.period = 100, qcov = v)
plot(fit8, "qq")
```
...
We must determine the threshold for each cluster. Notice, however, that some clusters contain very little data that consists of mostly extremes, so choosing a high threhsold for those clusters would be a mistake since we will not be implementing those data into the model. 
```{r}
...
```{r}
plot(fit10, type = "rl", rperiods = c(100))
```
...
```{r}
return.level(fit10, return.period = c(2, 20, 50, 75, 100), qcov=v)[1, ]
```

```{r}
fit1_ci2 = ci(fit1, type="return.level", method="proflik", return.period = 2, xrange = c(11.9, 13), verbose = TRUE)
fit1_ci20 = ci(fit1, type="return.level", method="proflik", return.period = 20, xrange = c(13, 17), verbose = TRUE)
fit1_ci50 = ci(fit1, type="return.level", method="proflik", return.period = 50, xrange = c(14, 18), verbose = TRUE)
fit1_ci75 = ci(fit1, type="return.level", method="proflik", return.period = 75, xrange = c(14, 18), verbose = TRUE)
fit1_ci100 = ci(fit1, type="return.level", method="proflik", return.period = 100, xrange = c(14.5, 18), verbose = TRUE)
```

```{r}
set.seed(123)
summer_boot = function(data, return_period) {
  sample_data = data[sample(1:nrow(data), replace = TRUE), ]
  non_summer_wind = sample_data %>% filter(summer_index == 0)
  summer_wind = sample_data %>% filter(summer_index == 1)
  
  prob = 0.95
  t0 = quantile(non_summer_wind$surface_wind, prob = prob)
  t1 = quantile(summer_wind$surface_wind, prob = prob)
  fit = fevd(surface_wind, sample_data, threshold = c(t0, t1-t0), threshold.fun = ~ summer_index,
            location.fun = ~ summer_index, scale.fun = ~ summer_index, use.phi = TRUE, 
            shape.fun = ~ summer_index, type="PP")
  v = make.qcov(fit8, vals = list(threshold = c(t0, t1-t0),
                                mu1 = c(0, 1),
                                phi1 = c(0, 1),
                                xi1 = c(0, 1)))
...
results <- foreach(i = c(2, 20, 50, 75, 100), .combine = rbind, 
  ...
  replicate(1000, summer_boot(wind_speed, i))
}
...
```{r}
rl_nonsummer_2 = results[1, ][seq(1, length(results[1, ]), by = 2)]
rl_summer_2 = results[1, ][seq(2, length(results[1, ]), by = 2)]
rl_nonsummer_20 = results[2, ][seq(1, length(results[2, ]), by = 2)]
rl_summer_20 = results[2, ][seq(2, length(results[2, ]), by = 2)]
rl_nonsummer_50 = results[3, ][seq(1, length(results[3, ]), by = 2)]
rl_summer_50 = results[3, ][seq(2, length(results[3, ]), by = 2)]
rl_nonsummer_75 = results[4, ][seq(1, length(results[4, ]), by = 2)]
rl_summer_75 = results[4, ][seq(2, length(results[4, ]), by = 2)]
rl_nonsummer_100 = results[5, ][seq(1, length(results[5, ]), by = 2)]
rl_summer_100 = results[5, ][seq(2, length(results[5, ]), by = 2)]

lower_ci2 = c(quantile(rl_nonsummer_2, 0.025), quantile(rl_summer_2, 0.025))
upper_ci2 = c(quantile(rl_nonsummer_2, 0.975), quantile(rl_summer_2, 0.975))

lower_ci20 = c(quantile(rl_nonsummer_20, 0.025), quantile(rl_summer_20, 0.025))
upper_ci20 = c(quantile(rl_nonsummer_20, 0.975), quantile(rl_summer_20, 0.975))

lower_ci50 = c(quantile(rl_nonsummer_50, 0.025), quantile(rl_summer_50, 0.025))
upper_ci50 = c(quantile(rl_nonsummer_50, 0.975), quantile(rl_summer_50, 0.975))

lower_ci75 = c(quantile(rl_nonsummer_75, 0.025), quantile(rl_summer_75, 0.025))
upper_ci75 = c(quantile(rl_nonsummer_75, 0.975), quantile(rl_summer_75, 0.975))

lower_ci100 = c(quantile(rl_nonsummer_100, 0.025), quantile(rl_summer_100, 0.025))
upper_ci100 = c(quantile(rl_nonsummer_100, 0.975), quantile(rl_summer_100, 0.975))
```

```{r}
temp = return.level(fit8, return.period = c(2, 20, 50, 75, 100), qcov=v)
temp1 = data.frame(
  x = rep(1:2, 5),
  y = c(temp[, 1], temp[, 2], temp[, 3], temp[, 4], temp[, 5]),
  group = rep(c("2 year Return Period", "20 year Return Period", "50 year Return Period",
                "75 year Return Period", "100 year Return Period"), each = 2),
...
ggplot(temp1, aes(x = x, y = y, group = group)) +
  ...
  labs(title = "Bootstrapped Conditional Return Level Plot", x = "Summer Index", y = "Wind Speed (m/s)", 
       fill = "Return Period", color = "Return Period") +
  scale_x_continuous(breaks = 1:2, labels = c("Non-summer", "Summer")) +
  theme_minimal() + 
  theme(
    legend.position = c(0.82, 0.85),  # x and y coordinates inside plot
    legend.background = element_rect(fill = "white", color = "black")  # optional styling
  )
```
...
```{r}
clusters = c(1, 2)
prop = c(1-mean(wind_speed$summer_index, mean(wind_speed$summer_index)))

cond_rl = function(z, k, fit) {
  params = fit$results$par
  if(k != 1) {
    mu = params["mu0"] + params["mu1"]
    sigma = exp(params["phi0"] + params["phi1"])
    xi = params["xi0"] + params["xi1"]
  } else {
    mu = params["mu0"]
...
return_period = 100
uniroot(function(z) joint_rl(z, fit8) - 1/return_period, lower = 0, upper = 100)$root
```
...
summer_joint_rl = function(data, return_period) {
  sample_data = data[sample(1:nrow(data), replace = TRUE), ]
  non_summer_wind = sample_data %>% filter(summer_index == 0)
  summer_wind = sample_data %>% filter(summer_index == 1)
...
results <- foreach(i = c(2, 20, 50, 75, 100), .combine = rbind, 
                   .packages = c("dplyr", "extRemes")) %dopar% {
  replicate(1000, summer_joint_rl(wind_speed, i))
}
...
return_period = 100
uniroot(function(z) joint_rl(z, fit12) - 1/return_period, lower = 0, upper = 100)$root
```

```{r}
hist(cluster0$surface_wind, breaks = 25)
abline(v = 10, col = "red", lwd = 2, lty = 2)
hist(cluster1$surface_wind, breaks = 25)
abline(v = 10, col = "red", lwd = 2, lty = 2)
hist(cluster2$surface_wind, breaks = 25)
abline(v = 10, col = "red", lwd = 2, lty = 2)
hist(cluster3$surface_wind, breaks = 25)
abline(v = 10, col = "red", lwd = 2, lty = 2)
hist(cluster4$surface_wind, breaks = 25)
abline(v = 10, col = "red", lwd = 2, lty = 2)
hist(cluster5$surface_wind, breaks = 25)
abline(v = 10, col = "red", lwd = 2, lty = 2)
hist(cluster6$surface_wind, breaks = 25, xlim = c(10, 16))
abline(v = 10, col = "red", lwd = 2, lty = 2)
```
...
legend('topright', legend=c("Unconditional (mixed)", "Stationary fit"), lty=c(1,1))
```
...
estimate = c(uniroot(function(z) joint_rl(z, fit8) - 1/(2), lower = 0, upper = 100)$root,
             uniroot(function(z) joint_rl(z, fit8) - 1/(20), lower = 0, upper = 100)$root,
             uniroot(function(z) joint_rl(z, fit8) - 1/(50), lower = 0, upper = 100)$root,
             uniroot(function(z) joint_rl(z, fit8) - 1/(75), lower = 0, upper = 100)$root,
             uniroot(function(z) joint_rl(z, fit8) - 1/(100), lower = 0, upper = 100)$root)
temp_df = data.frame(
