...
```{r}
yearly_max_climatology_hist = wind_speed %>% group_by(day_of_year) %>% 
  summarise(max_wind_speed = max(surface_wind))
yearly_max_climatology_future = clim_wind %>% group_by(day_of_year) %>% 
  summarise(max_wind_speed = max(surface_wind))
...
temp_long = data.frame(
  ...
  group = rep(c("Historical (1979-2020)", "Future (2025-2066)"), each = 366)
)
...
```{r}
temp = subset(wind_speed, select = -c(pressure_level, latitude, longitude, valid_time, Year, 
                                      Month, day_of_year, summer_index, cluster1, cluster2, cluster3,
                                      cluster4, cluster5, cluster6))
temp1 = subset(clim_wind, select = -c(X, Year, Month, day_of_year, time))
temp = temp %>% rename(low_level_wind = low.level_wind) %>% mutate(period = "Historical")
temp1 = temp1 %>% mutate(period = "Future")
